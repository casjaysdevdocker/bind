...
[ -z "$TOR_APPNAME_ENABLED" ] || if [ "$TOR_APPNAME_ENABLED" != "yes" ]; then export SERVICE_DISABLED="$SERVICE_NAME" && __script_exit 0; fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
EXEC_CMD_ARGS='-f $ETC_DIR/torrc' # command arguments
